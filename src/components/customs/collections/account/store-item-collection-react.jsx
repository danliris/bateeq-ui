...
import StoreItemReact from './store-item-react.jsx';

'use strict';

export default class StoreItemCollectionReact extends React.Component {
    constructor(props) {
        super(props);
    ...
    render() {
        var items = (this.state.value || []).map(function (item, index) {
            ...
            return (
                <StoreItemReact key={"__item" + index} value={item} error={error} options={this.state.options} onRemove={this.handleItemRemove} onChange={this.handleItemChange}></StoreItemReact>
            );
        }.bind(this))
