...
export class RestService {
  ...
  getXls(endpoint, header) {
    var request = {
      method: "GET",
      headers: new Headers(
        Object.assign({}, this.header, header, { Accept: "application/xls" })
      ),
    };
  ...
  getPdf(endpoint, header) {
    var request = {
      method: "GET",
      headers: new Headers(
        Object.assign({}, this.header, header, { Accept: "application/pdf" })
      ),
    };
    var getRequest = this.endpoint.client.fetch(endpoint, request);
    this.publish(getRequest);
  ...
  _downloadFile(request) {
    return request
      .then((response) => {
        ...
        else return Promise.reject(new Error("Error downloading file"));
      })
      .then((response) => {
