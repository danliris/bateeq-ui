...
export class Report {
    ...
    async activate() {
    let dateFrom = moment(new Date()).subtract(1, 'days').startOf('days');
   let dateTo = moment(new Date()).subtract(1, 'days').endOf('days');


   
      //let dateFrom = moment(new Date('2017-05-10'));
      //let dateTo = moment(new Date('2017-05-10'));
     //  let dateFrom = new ISODate('2013-12-12');
      // let dateTo = new ISODate('2013-12-12');
     // dateFrom.setDate(dateFrom.getDate() - 1); 
     // dateTo.setDate(dateTo.getDate() - 1); 
       let apiResult = await this.service.getSalesSummary(dateFrom.format(), dateTo.format())
//let apiResult = await this.service.getSalesSummary(ISODate('05-10-2017').format(),ISODate('05-10-2017').format())
            .then(result => {
                return result;
            });
